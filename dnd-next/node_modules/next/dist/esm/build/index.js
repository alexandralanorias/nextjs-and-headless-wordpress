...
import { STATIC_STATUS_PAGE_GET_INITIAL_PROPS_ERROR, PUBLIC_DIR_MIDDLEWARE_CONFLICT, MIDDLEWARE_FILENAME, PAGES_DIR_ALIAS, INSTRUMENTATION_HOOK_FILENAME, NEXT_DID_POSTPONE_HEADER, RSC_PREFETCH_SUFFIX, RSC_SUFFIX } from "../lib/constants";
...
import { BUILD_ID_FILE, BUILD_MANIFEST, CLIENT_STATIC_FILES_PATH, EXPORT_DETAIL, EXPORT_MARKER, FONT_MANIFEST, IMAGES_MANIFEST, PAGES_MANIFEST, PHASE_PRODUCTION_BUILD, PRERENDER_MANIFEST, REACT_LOADABLE_MANIFEST, ROUTES_MANIFEST, SERVER_DIRECTORY, SERVER_FILES_MANIFEST, STATIC_STATUS_PAGES, MIDDLEWARE_MANIFEST, APP_PATHS_MANIFEST, APP_PATH_ROUTES_MANIFEST, APP_BUILD_MANIFEST, RSC_MODULE_TYPES, NEXT_FONT_MANIFEST, SUBRESOURCE_INTEGRITY_MANIFEST, MIDDLEWARE_BUILD_MANIFEST, MIDDLEWARE_REACT_LOADABLE_MANIFEST, SERVER_REFERENCE_MANIFEST, FUNCTIONS_CONFIG_MANIFEST } from "../shared/lib/constants";
...
import { detectConflictingPaths, computeFromManifest, getJsPageSizeInKb, printCustomRoutes, printTreeView, copyTracedFiles, isReservedPage, isAppBuiltinNotFoundPage } from "./utils";
...
import { lockfilePatchPromise, teardownTraceSubscriber, teardownCrashReporter, loadBindings, teardownHeapProfiler, createDefineEnv } from "./swc";
import { getNamedRouteRegex } from "../shared/lib/router/utils/route-regex";
import { flatReaddir } from "../lib/flat-readdir";
import { eventSwcPlugins } from "../telemetry/events/swc-plugins";
import { normalizeAppPath } from "../shared/lib/router/utils/app-paths";
import { ACTION, NEXT_ROUTER_PREFETCH_HEADER, RSC_HEADER, RSC_CONTENT_TYPE_HEADER, RSC_VARY_HEADER } from "../client/components/app-router-headers";
...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, turboNextBuildRoot = null, buildMode) {
    ...
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            await nextBuildSpan.traceChild("write-routes-manifest").traceAsyncFn(()=>fs.writeFile(routesManifestPath, formatManifest(routesManifest), "utf8"));
            ...
            async function turbopackBuild() {
                ...
                let binding = await loadBindings(config == null ? void 0 : (_config_experimental = config.experimental) == null ? void 0 : _config_experimental.useWasmBinary);
            ...
            if (!isGenerate) {
                if (isCompile && config.experimental.webpackBuildWorker) {
                    ...
                    const { duration: webpackBuildDuration, ...rest } = turboNextBuild ? await turbopackBuild() : await webpackBuild();
            ...
            if (appDir) {
                ...
                await fs.writeFile(path.join(distDir, APP_PATH_ROUTES_MANIFEST), formatManifest(appPathRoutes), "utf-8");
            }
            ...
            const appStaticWorkers = appDir ? createStaticWorker(incrementalCacheIpcPort, incrementalCacheIpcValidationKey) : undefined;
                                ...
                                try {
                                    ...
                                    if (pageType === "app" && originalAppPath) {
                                        ...
                                        if (isEdgeRuntime(pageRuntime)) {
                                            ...
                                            // an app page.
                                            if (!isStatic && !isAppRouteRoute(originalAppPath) && !isDynamicRoute(originalAppPath) && !isPPR) {
            ...
            if (Object.keys(functionsConfigManifest).length > 0) {
                ...
                await fs.writeFile(path.join(distDir, SERVER_DIRECTORY, FUNCTIONS_CONFIG_MANIFEST), formatManifest(manifest), "utf8");
            }
            ...
            if (serverPropsPages.size > 0 || ssgPages.size > 0) {
                ...
                await fs.writeFile(routesManifestPath, formatManifest(routesManifest), "utf8");
            }
            ...
            await fs.writeFile(path.join(distDir, SERVER_FILES_MANIFEST), formatManifest(requiredServerFiles), "utf8");
            ...
            if (!isCompile && (combinedPages.length > 0 || useStaticPages404 || useDefaultStatic500 || appDir)) {
                ...
                await staticGenerationSpan.traceAsyncFn(async ()=>{
                    ...
                    await fs.writeFile(manifestPath, formatManifest(pagesManifest), "utf8");
                });
            }
            ...
            if (ssgPages.size > 0 || appDir) {
                ...
                await fs.writeFile(path.join(distDir, PRERENDER_MANIFEST), formatManifest(prerenderManifest), "utf8");
            ...
            await fs.writeFile(path.join(distDir, IMAGES_MANIFEST), formatManifest({
                version: 1,
                images
            }), "utf8");
            await fs.writeFile(path.join(distDir, EXPORT_MARKER), formatManifest({
                version: 1,
            ...
            if (config.analyticsId) {
                console.log(bold(green("Next.js Speed Insights")) + " is enabled for this production build. " + "You'll receive a Real Experience Score computed by all of your visitors.");
                console.log("");
            }
            ...
            if (config.output === "standalone") {
                await nextBuildSpan.traceChild("copy-traced-files").traceAsyncFn(async ()=>{
                    await copyTracedFiles(dir, distDir, pageKeys.pages, denormalizedAppPages, outputFileTracingRoot, requiredServerFiles.config, middlewareManifest, hasInstrumentationHook, staticPages);
                    if (config.output === "standalone") {
                        for (const file of [
